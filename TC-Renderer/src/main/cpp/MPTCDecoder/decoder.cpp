...
//    decode_info->is_start = false;
    ...
    ///USE NEW. FULL C++
    decode_info->comp_palette = new uint8_t[decode_info->max_compressed_palette];
    decode_info->uncomp_palette = new uint8_t[decode_info->max_unique_count];
    decode_info->comp_motion_indices = new uint8_t[decode_info->max_compressed_motion_indices];
    decode_info->motion_indices = new uint8_t[2 * decode_info->num_blocks];
    decode_info->comp_ep_Y = new uint8_t[decode_info->max_compressed_ep_Y];
    decode_info->comp_ep_C = new uint8_t[decode_info->max_compressed_ep_C];

    decode_info->wav_ep1_Y = new uint8_t[decode_info->num_blocks];
    decode_info->wav_ep1_C = new uint8_t[2 * decode_info->num_blocks];

    decode_info->wav_ep2_Y = new uint8_t[decode_info->num_blocks];
    decode_info->wav_ep2_C = new uint8_t[(2 * decode_info->num_blocks)];

    decode_info->ep1_Y = new int8_t[(decode_info->num_blocks)];
    decode_info->ep2_Y = new int8_t[(decode_info->num_blocks)];
    decode_info->ep1_Co = new int8_t[(decode_info->num_blocks)];
    decode_info->ep2_Co = new int8_t[(decode_info->num_blocks)];
    decode_info->ep1_Cg = new int8_t[(decode_info->num_blocks)];
    decode_info->ep2_Cg = new int8_t[(decode_info->num_blocks)];
...
//    }
    ...
    if(decode_info->is_multi_thread) {
      decode_info->comp_ep1_Y = new uint8_t[(decode_info->max_compressed_ep_Y)];
      decode_info->comp_ep1_C = new uint8_t[(decode_info->max_compressed_ep_C)];
      decode_info->comp_ep2_Y = new uint8_t[(decode_info->max_compressed_ep_Y)];
      decode_info->comp_ep2_C = new uint8_t[(decode_info->max_compressed_ep_C)];
    }
...
int InitBufferedDecode(uint8_t buffer_sz, 
  ...
  for(uint8_t idx = 0; idx < buffer_sz; idx++) {
    ptr_buffer_struct->buffered_dxts =
        (PhysicalDXTBlock **) malloc(buffer_sz * sizeof(PhysicalDXTBlock *));
  }
...
//  for(uint8_t idx = 0; idx < buffer_sz; idx++)
//    ptr_buffer_struct->buffered_dxts = new PhysicalDXTBlock*[buffer_sz];
//
//  for(uint8_t idx = 0; idx < buffer_sz; idx++){
//    ptr_buffer_struct->buffered_dxts[idx] = new PhysicalDXTBlock[num_blocks];
//  }
  ...
  ptr_buffer_struct->prev_decode_idx = ptr_buffer_struct->buffer_sz - 2;
...
int GetBufferedFrame(BufferStruct *ptr_buffer_struct, PhysicalDXTBlock * &curr_dxt, std::ifstream  &in_stream) {
  ...
  else LOGE("Get Frame MultiThread not joinable!\n");
